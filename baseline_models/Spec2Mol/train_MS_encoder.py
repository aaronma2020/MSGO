...
def train(args, train_loader, valid_loader, model, epochs):
    ...
    for epoch in range(epochs):
        ...
        losses.append([np.mean(epoch_losses), valid_loss])
        loss_df = pd.DataFrame(losses)
        epoch_losses = []

        if epoch > 1:
            os.remove('lipid_loss.csv')
        loss_df.to_csv('lipid_loss.csv')
        if epoch>49 and epoch%5==0 and args.save_models:
            model_dir = os.path.join(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_spectra_encoder_model", str(epoch) + '.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-neg', type=bool, default=False, help='Use negative mode spectra if true')
    parser.add_argument('-emb_dim', type=int, default=512, help='Dimensionality of the embedding space')
    parser.add_argument('-augm', type=bool, default=False, help='Perorm data augmentation if true')
