...
from decoder.moses.interfaces import MosesTrainer
from decoder.moses.utils import (
    SmilesOneHotVocab,
    Logger,
    smiles_tokenize,
...
from decoder.moses.trans.misc import CosineAnnealingLRWithRestart, KLAnnealer
...
class TranslationTrainer(MosesTrainer):
    ...
    def _train_epoch(
        ...
        for i, input_batch in enumerate(tqdm_data):
            ...
            # print(i)
            randomized_smiles, canonical_smiles, regression_targets = input_batch['x'], input_batch['label1'], input_batch['label2']
            ...
            regression_loss = regression_loss.to('cuda:2')
            ...
            if i % self.config.tb_log_interval == 0:
                ...
                if i % self.config.save_frequency == 0:
                    pct_match = compute_reconstruction_trans(model, tqdm_data)
                    ...
                    if pct_match > self.config.best_pct_match:
                        ...
                        torch.save(
                            model.state_dict(),
                            self.config.model_save + "\model" + str(epoch) + ".pt",
                        )
                        torch.save(
                            optimizer.state_dict(),
                            self.config.model_save + "\opt" + str(epoch) + ".pt",
                        )
                        torch.save(
                            postfix,
                            self.config.model_save + "\postfix" + str(epoch) + '-' + str(postfix['step']) + ".pt",
                        )
