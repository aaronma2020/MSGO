...
class MSDataset_4channels(torch.utils.data.Dataset):
    ...
    def __init__(self, partition, resolution, neg, augm=False, train_on_valid=False):
        ...
        all_smiles = [i[0] for i in dict(
            torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_smiles_high_energy_dict.pt")).items()]

        global smiles
        if partition != 'experiment':
            self.smiles_emb = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_smiles_encode_dict.pt")
            smiles_spec_MHposlow = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_smiles_low_energy_dict.pt")
            smiles_spec_MHposhigh = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_smiles_high_energy_dict.pt")

            with open(r"E:\BQY\模型复现\lipid\Spec2Mol\data.json", 'r') as f:
                data_inf = json.load(f)
                raw_smiles_dict = data_inf['gts']
                if partition == 'train':
                    smiles = [raw_smiles_dict[str(i)] for i in data_inf['data']['train']]
                elif partition == 'valid':
                    smiles = [raw_smiles_dict[str(i)] for i in data_inf['data']['val']]
                elif partition == 'test':
                    smiles = [raw_smiles_dict[str(i)] for i in data_inf['data']['test']]
            smiles = [i for i in smiles if i in all_smiles]
        else:
            self.smiles_emb = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_experiment_smiles_encode_dict.pt")
            smiles_spec_MHposlow = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_experiment_smiles_low_energy_dict.pt")
            smiles_spec_MHposhigh = torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_experiment_smiles_high_energy_dict.pt")
            smiles = [i[0] for i in dict(
                torch.load(r"E:\BQY\PycharmProjects\Spec2Mol\lipid_smiles_encode_data\lipid_experiment_smiles_high_energy_dict.pt")).items()]

            example = list(dict(self.smiles_emb).items())[0][1]
            for smi in smiles:
                self.smiles_emb[smi] = torch.zeros_like(example)

        smiles_spec_MHneglow = {}
        smiles_spec_MHneghigh = {}
        smiles_spec_MHposlow_augm = {}
        smiles_spec_MHposhigh_augm = {}
        smiles_spec_MHneglow_augm = {}
        smiles_spec_MHneghigh_augm = {}
    ...
    def __getitem__(self, idx):
        ...
        '''
        embedding = self.smiles_emb[smiles]
